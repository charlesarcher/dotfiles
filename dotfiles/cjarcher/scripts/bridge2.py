...
def toggle_mumble_ptt(key_down):
    action = "key --down 311" if key_down else "key --up 311"
    try:
        subprocess.run(["ydotool", action], check=True)
        print(f"ydotool command executed: {action}")
    except subprocess.CalledProcessError as e:
        print(f"Error executing ydotool: {e}")
            ...
            if event.type == ecodes.EV_KEY:
                ...
                if event.code == ptt_key:
                    if event.value == 1:  # Key press
                        ...
                        toggle_mumble_ptt(True)
                    elif event.value == 0:  # Key release
                        print("PTT key released")
                        toggle_mumble_ptt(False)
